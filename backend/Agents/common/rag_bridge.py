...
def query(owner: str, repo: str, question: str, limit: int) -> Dict[str, Any]:
    model = embedder()
    return {"hits": memory_store.retrieve(owner, repo, model.embed_query(question), k=limit)}
...
def main() -> None:
    for line in sys.stdin:
        ...
        if operation == "upsert":
            ...
            result = query(request["owner"], request["repo"], request["question"], int(request.get("limit") or 8))
        else:
