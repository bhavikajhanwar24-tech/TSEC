...
async function saveAgentRun(issueRecord, state, agentName, result, status = "complete") {
  ...
  const values = {
    ...
    triggeringEvent: {
      type: "github_issue_workflow",
      sourceId: issueRecord.githubIssueId,
      timestamp: new Date().toISOString(),
      repoFullName: issueRecord.repoFullName,
    },
    toolCalls: output.tool_calls || output.toolCalls || [],
    reasoningTrace: output.reasoning_steps || output.reasoningSteps || [output.recommendation || output.report || "Agent completed."],
    finalAction: output.final_action || output.suggested_action || output.action || null,
